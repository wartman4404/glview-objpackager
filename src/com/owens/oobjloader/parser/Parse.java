...
public class Parse {
    ...
    private final static String MTL_BUMP = "map_Bump";
    ...
    public static class FilesystemLoader implements MaterialLoader {
    	private final File objFile;
    	
    	public FilesystemLoader(String filename) {
    		objFile = new File(filename);
    	}

		@Override
		public InputStream getMaterialStream(String name) throws FileNotFoundException {
			File mtlFile = new File(objFile.getParent(), name);
			return new FileInputStream(mtlFile);
		}
    }
    ...
    public Parse(BuilderInterface builder, String filename) throws FileNotFoundException, IOException {
        ...
        this.loader = new FilesystemLoader(filename);
        parseObjFile(new FileInputStream(filename));

        builder.doneParsingObj();
    }
    
    public Parse(BuilderInterface builder, InputStream infile, MaterialLoader loader) throws IOException {
    	this.builder = builder;
    	this.loader = loader;
    	parseObjFile(infile);
    ...
    private void parseObjFile(InputStream infile) throws FileNotFoundException, IOException {
        int lineCount = 0;
...
//        System.err.println("Parsed as "+parsedList);
    ...
    private void processMaterialLib(String line) throws FileNotFoundException, IOException {
        ...
        if (null != matlibnames) {
            for (int loopi = 0; loopi < matlibnames.length; loopi++) {
                try {
                	parseMtlFile(loader.getMaterialStream(matlibnames[loopi]));
    ...
    // ----------------------------------------------------------------------
    private void parseMtlFile(InputStream matStream) throws FileNotFoundException, IOException {
        int lineCount = 0;
        BufferedReader bufferedReader = null;
        
        bufferedReader = new BufferedReader(new InputStreamReader(matStream));
